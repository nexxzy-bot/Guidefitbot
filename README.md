...
Текущая версия — **2.7.4** (берётся из `package.json`, показывается в «О приложении» и `/api/health`; `versionCode` APK = 274).

**Что нового в 2.7.4** (аудит безопасности + производительность)

- Ошибки БД больше не отдают наружу `err.message` (было в ~60 ответах 500): клиент видит
  общий текст «Database error», код и стек остаются в журнале сервера.
- Публичные `:id`-эндпоинты (`/api/recipe/:id`, `/api/exercise/:id`, `/api/program/:id`,
  `/api/fit/program/:id`, `/api/yoga2/program/:id`, `/api/yoga/flow/:id`,
  `/api/recipe-image/:id`, `/api/workout/log/:id`, `/api/exercise-media/:id`) отсекают
  нечисловые идентификаторы до запроса (раньше «abc»/«1 OR 1=1» уезжали параметром —
  не инъекция, но бессмысленное сканирование таблицы).
- Поиск в админке экранирует LIKE-символы (`%`, `_`) с `ESCAPE '\\'` — запрос «%» больше
  не сканирует всю таблицу и не ломает пагинацию.
- Поисковые запросы к Pexels санитизируются (переносы строк и табуляция → пробел) —
  защита от подмены заголовков HTTP-запроса данными каталога.
- На гостевой `POST /api/meal` добавлен лимит 30 req/min на IP (эндпоинт гоняет SELECT
  по каталогу на 2000 блюд с `ORDER BY RANDOM()`; авторизованные остаются под общим лимитом).
- Статика отдаётся с осмысленным `Cache-Control`: шрифты/фото/GIF — `immutable` 1 год,
  HTML/SW/манифест — `no-cache`.
- SQLite: добавлены покрывающие индексы `water_logs(tg_id, date, amount_ml)` и
  `weight_logs(tg_id, date, weight)` (серии достижений читали таблицу поверх уникального
  индекса) и `user_programs(tg_id, active)` (активная программа легаси на дашборде).
- `/api/fit/program/:id` и `/api/yoga2/program/:id`: вместо 2N+1 последовательных запросов
  (упражнения + GIF на каждый день/позу) — 3 запроса на программу. Заодно исправлено
  поле `gif_url` йоги: наружу уходила сырая JSON-строка из meta вместо пути к файлу
  (фронт это компенсировал отдельными запросами на `/api/exercise-media`).
- `last_seen` обновляется не чаще раза в 10 минут (было: UPDATE почти на каждый запрос —
  лишние записи в WAL); каталог достижений читается из БД раз в 5 минут, а не на каждый
  пересчёт серий.
- Проверены и подтверждены: у каждого пользователя уникальный внутренний ключ `tg_id`
  (`anon:…`/`vk:…`, PK таблицы users); все запросы строятся на подготовленных
  выражениях (SQL-инъекции исключены); `.env` и база не в git; `requireAdmin` сравнивает
  токен через `crypto.timingSafeEqual`; тело запроса ограничено 1 МБ; диапазоны числовых
  полей проверяются на входе.
...
- Разовая очистка «осиротевших» записей дневника (food_logs с recipe_id из старого
  каталога блюд, которого больше нет) — 12 строк, искажавших счётчики достижений,
  админки и выгрузку данных. Новые невозможны: `/api/log-meal` проверяет рецепт.
