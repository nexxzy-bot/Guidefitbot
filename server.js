...
app.get('/api/recipe-image/:id', async (req, res) => {
  db.get("SELECT title, image_url FROM recipes WHERE id = ?", [req.params.id], async (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    ...
    const url = await fetchPexelsPhoto(row.title + ' food dish meal');
    if (url) {
...
app.post('/api/water', (req, res) => {
  ...
  if (!amount) return res.status(400).json({ error: 'Missing fields' });
  const today = localDate();
...
app.post('/api/weight', (req, res) => {
  ...
  if (!weight) return res.status(400).json({ error: 'Missing fields' });
